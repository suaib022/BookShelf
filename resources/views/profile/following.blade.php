            ...
            @foreach($following as $followee)
                <div class="flex items-center justify-between p-4 bg-white border border-[#E8E6E0] rounded-md shadow-sm">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <a href="{{ route('profile.show', $followee->username) }}" class="font-bold text-[#382110] hover:text-[#00635D] leading-tight block">
                                {{ $followee->name }}
                            </a>
                            <p class="text-xs text-[#888]">{{ '@' . $followee->username }}</p>
                        </div>
                    </div>
                    @if(auth()->check() && auth()->id() !== $followee->id)
                        @php
                            $isFollowing = auth()->user()->following()->where('followee_id', $followee->id)->exists();
                        @endphp
